...
const LocalPurchaseOrderFormTable = ({ records }: { records: any }) => {
  return (
    <DataTable
      minHeight={150}
      records={records ?? []}
      columns={[
        ...
        },
        {
          ...
          width: 80,
          render: (entry: any) => (
            <Anchor component={Link} href={`/account/forms/local-purchase-order-forms/view/${entry?.id}/`}>
...
const MyLocalPurchaseOrderForms = (props: IMyForms) => {
  const { my_forms, userId, forms_to_approve } = props
  ...
  return (
    <div>
      <Container size="lg" py={50}>
        ...
        <Stack>
          <Title>My Local Purchase Order Forms</Title>
          <LocalPurchaseOrderFormTable records={my_forms} />
        </Stack>

        {/* {
          user_ ? user_?.profile?.checker ? (
            <Stack>
              <Title>Local Purchase Order Forms to Check</Title>
              <LocalPurchaseOrderFormTable records={forms_to_check} />
            </Stack>
          ) : null : null
        } */}

            <Stack>
              <Title>Local Purchase Order Forms to Approve</Title>
              <LocalPurchaseOrderFormTable records={forms_to_approve} />
            </Stack>
...
export const getServerSideProps = async (context: any) => {
  ...
  const forms_to_approve_query = await makeRequestOne({
    ...
    params: {
      ...
      limit: 100,
      fields: "id,currency,requested_by,user,full_name,level,invoice_number,supplier,project,code,name,total,created_on"
    }
  })
  ...
  return Promise.allSettled([my_forms_query, forms_to_approve_query]).then((res) => {
...
MyLocalPurchaseOrderForms.PageLayout = HeaderAndFooterWrapper
export default MyLocalPurchaseOrderForms